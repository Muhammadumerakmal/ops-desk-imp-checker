...
- [ ] T1.1 — `src/config.py`: load `.env`; clear error if `OPENAI_API_KEY` missing
...
- [ ] T1.2 — `courses.json` + `src/course_store.py`: load and query courses. (FR-2)
  - Verify: `list_courses` reflects file edits; unknown id → sentence, not crash.
- [ ] T1.3 — `src/models.py`: `Ticket` pydantic model. (FR-7)
- [ ] T1.4 — `src/profiles.py`: `StudentProfile` dataclass + three sample profiles. (FR-3)
- [ ] T1.5 — `src/tools.py` (data + profile tools): `list_courses`, `get_course`,
  `lookup_assignment`, `get_my_profile(ctx)`. (FR-2, FR-3)
  - Verify: `get_my_profile` schema has **no** `ctx`/wrapper param.
- [ ] T1.6 — `src/instructions.py`: dynamic instructions from profile — greet by
...
- [ ] T1.7 — `src/desk.py` (Desk only) + `main.py`: async entry, `asyncio.run`, a
...
- [ ] T2.1 — `src/desk.py`: `base_specialist`; clone → Assignments (cold) & Careers
  (warm); register handoffs on the Desk. (FR-5)
  - Verify: `result.last_agent` names the specialist; handoff item present.
- [ ] T2.2 — `src/desk.py`: Summariser agent → `.as_tool()` on the Desk. (FR-6)
  - Verify: final message after summarising still comes from the Desk.
- [ ] T2.3 — Desk `output_type=Ticket`; branch on `resolved` in Python. (FR-7)
  - Verify: `type(final_output) is Ticket`; impossible request → parsing error.
- [ ] T2.4 — `src/guardrails.py`: input guardrail refusing off-topic; caught in
  `main.py`/`app.py`. (FR-8)
  - Verify: off-topic → courteous refusal, no Desk model call, no crash.
- [ ] T2.5 — `src/tools.py`: scholarship-only tool (`is_enabled`); `close_ticket`
...
- [ ] T3.1 — `src/hooks.py`: `AuditRunHooks` (run-level) → `audit_log.jsonl`;
  `SpecialistAgentHooks` on Assignments only. (FR-10, NFR-3)
  - Verify: one question → one timeline naming both agents in order.
- [ ] T3.2 — `src/runner.py`: custom runner stamping request-id + elapsed, registered
  once at startup; no agent file mentions it. (FR-11)
  - Verify: wrapper output appears for Desk and specialist runs.
- [ ] T3.3 — `app.py`: Chainlit; agent + profile built once per session; memory;
  `await` the run. (FR-12)
  - Verify: second message understood; two windows isolated.
- [ ] T3.4 — tracing on, exported under our key, one trace per conversation. (FR-13)
  - Verify: open the trace; every span nameable.

## Demo

- [ ] T4.1 — one clean conversation → one trace → one ticket. (whole project)
